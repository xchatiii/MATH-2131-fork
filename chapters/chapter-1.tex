...
\begin{definition}[Unital]
	A binary operation $\cdot: S \times S \to S$ is said to be \emph{unital}\index{unital} if there exists an element $e \in S$, the \emph{identity}\index{identity} or \emph{unit element}, such that, for all $x \in S$,
	\[ e \cdot x = x = x \cdot e \]
\end{definition}
