...
\begin{definition}[Identifiable]
	A binary operation $\cdot: S \times S \to S$ is said to be \emph{identifiable}\index{identifiable}, or \emph{unital}\index{unital}, if there exists an element $e \in S$, the \emph{identity}\index{identity} or \emph{unit element}\index{unit element}, such that, for all $x \in S$
	\[ e \cdot x = x = x \cdot e \]
\end{definition}
...
\begin{definition}[Group Homomorphism]
	A \emph{group homomorphism}\index{group homomorphism} is a morphism between two groups that preserves the group structure.
...
\begin{definition}[Linear Structure]
	A \emph{linear structure}\index{linear structure} over a field $\F$ on a set $V$ is a pair $(+, \cdot)$ where $(V, +)$ is an abelian group with a ring homomorphism $\F \to \End(V)$, where $\End(V)$ is the endomorphism ring of the abelian group $(V, +)$.
	\begin{align*}
		\cdot : \F &\to \End(V) \\
		\alpha &\mapsto (\alpha\cdot : \vec x \mapsto \alpha \vec x) \qquad \alpha \cdot : V \to V
	\end{align*}
	The ring homomorphism is a (ring) action of the field $\F$ on the abelian group $(V, +)$, called \emph{scalar multiplication}\index{scalar multiplication}.
...
\begin{example}
	...
	\[
		...
		\right\}
	\]
	is the set of all \emph{column matrices}\index{column matrices} with $n$ entries in $\F$.
	Elements in $\F^n$ are written as $\vec x$ and are called \emph{column vectors}\index{column vectors}. $\F^n$ is a linear space over $\F$ with the following operations defined entrywise:
	\begin{align*}
		+ : \F^n \times \F^n &\to \F^n \\
