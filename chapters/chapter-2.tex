...
\begin{definition}[Linear Maps]
	...
	A \emph{linear map} is a set map $T: V \to W$ such that for all $u, v \in V$ and $\alpha \in \mathbb{k}$, the following holds:
	\begin{align*}
		T(u + v) &= T(u) + T(v) \\
		T(\alpha u) &= \alpha T(u)
	\end{align*}
...
\begin{definition}[Linear Functional / Homogeneous Linear Function]
	A linear map $f: \mathbb{k}^n \to \mathbb{k}$ is called a \emph{homogeneous linear function} or a \emph{linear functional} if for all $\alpha \in \mathbb{k}$ and $x \in \mathbb{k}^n$, the following holds:
	\[
		f(\alpha x) = \alpha f(x)
	\]
\end{definition}
...
\begin{definition}[Elementary Row Operations]
	...
	\end{enumerate}
	Each elementary row operation can be represented by \emph{left multiplication} of $A$ by an appropriate $m \times m$ matrix over $\mathbb{k}$.
...
\begin{definition}[Matrix Units]
	...
	The \emph{matrix unit} $E_i^j$ is the $m \times n$ matrix over $\mathbb{k}$ with $1$ in the $(i,j)$-th position and $0$ elsewhere, i.e.,
	\[
	...
	It can also be defined as $E_i^j = \vec{e}_i \hat{e}^j \in \M{m \times n}{\mathbb{k}}$ where $\vec{e}_i \in \mathbb{k}^m$ and $\vec{e}_j^T = \hat{e}^j \in (\mathbb{k}^n)^*$ are the $i$-th and $j$-th standard basis vectors, respectively.
...
\begin{remark}
	...
	\begin{align*}
		A\vec{e}_j &= \text{the } j\text{-th column of } A \in \mathbb{k}^n \\
		\hat{e}^iA &= \text{the } i\text{-th row of } A \in (\mathbb{k}^m)^* \\
	\end{align*}
...
\begin{definition}[Finite Dimensional Vector Spaces] \label{def:finite_dimensional_vector_space}
	A linear space $V$ over a field $\mathbb{k}$ is \emph{finite dimensional} if there exists a linear equivalence $T: V \to \mathbb{k}^n$ for some positive integer $n$.
...
\begin{definition}[Infinite Dimensional Vector Spaces]
	A linear space $V$ over a field $\mathbb{k}$ is \emph{infinite dimensional} if $V$ is not finite dimensional.
\end{definition}
...
\begin{definition}[Elementary Column Operations]
	...
	\end{enumerate}
	Each elementary column operation can be represented by \emph{right multiplication} of $A$ by an appropriate $n \times n$ matrix over $\mathbb{k}$.
	Note that all of them are invertible linear maps from $\mathbb{k}^{m \times n}$ to $\mathbb{k}^{m \times n}$.
\end{definition}
