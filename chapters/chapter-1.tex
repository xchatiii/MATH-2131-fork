...
\begin{definition}[Invertible]
	A binary operation $\cdot: S \times S \to S$ is said to be \emph{invertible}\index{invertible} if, for every element $x \in S$, there exists an element $y \in S$, called the two-sided \emph{inverse}\index{inverse} of $x$, denoted as $x^{-1}$, such that
	\[ x \cdot y = e = y \cdot x \]
...
Same for the inverse, one-sided need not be unique.
...
\begin{example}
	...
	Elements in $\F^n$ are written as $\vec{x}$ and are called \emph{column vectors}. $\F^n$ is a linear space over $\F$ with the following operations defined entrywisely:
	\begin{align*}
		+ : \F^n \times \F^n &\to \F^n \\
