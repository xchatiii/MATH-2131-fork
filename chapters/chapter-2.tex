...
\begin{definition}[Injective Lienar Maps]
	A linear map $f: V \to W$ between two linear spaces over a field $\mathbb{k}$ is said to be \emph{injective}\index{injective} (or one-to-one) if for all $u, v \in V$, the following holds:
	\[
		f(u) = f(v) \implies u = v
	\]
...
\begin{definition}[Finite Dimensional Vector Spaces] \label{def:finite_dimensional_vector_space}
	A linear space $V$ over a field $\mathbb{k}$ is said to be \emph{finite dimensional} if there exists a linear equivalence $T: V \to \mathbb{k}^n$ for some positive integer $n$.
...
\begin{definition}[Infinite Dimensional Vector Spaces]
	A linear space $V$ over a field $\mathbb{k}$ is said to be \emph{infinite dimensional} if $V$ is not finite dimensional.
\end{definition}
