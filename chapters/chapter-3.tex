...
\begin{definition}[Linear Subspaces]
	Let $W$ be a linear space over $\mathbb{k}$ and $V$ is a subset of $W$, denoted as $V \subset W$. $V$ is a \emph{linear subspace} of $W$ if $V$, with $+$ and $\cdot$ inherited from those of $W$, is a linear space.
\end{definition}
...
\begin{definition}[Linear Span]
	...
	The \emph{linear span} of $S$, denoted by $\Span_\mathbb{k} (S)$ or simply $\Span S$ or $\bar{S}$ or $\langle S \rangle$, is defined as the completion of $S$ inside $V$ under linear combinations.
\end{definition}
...
\begin{definition}[Linearly Independent Sets]
	...
	A subset $S \subseteq V$ is a \emph{linearly independent set} of vectors in $V$ if no elements in $S$ can be expressed as a linear combination of the finitely many other elements in $S$.
\end{definition}

\begin{definition}[Spanning Sets]
	Let $V$ be a linear space over a field $\mathbb{k}$.
	A subset $S \subseteq V$ is a \emph{spanning set} of $V$ if $\Span(S) = V$.
\end{definition}

\begin{example}
	Let $V = \mathbb{k}^3$ and consider the three vectors $\vec{e}_1$, $\vec{e}_2$ and $\vec{e}_3$.
...
\end{example}
\begin{remark}
	If we consider the matrix of $\{\vec{e}_1, \vec{e}_2, \vec{e}_1 + \vec{e}_2\}$ with respect to the standard basis of $\mathbb{k}^3$, we have:
	\[
		A = \begin{bmatrix}
			1 & 0 & 1 \\
			0 & 1 & 1 \\
...
\begin{definition}[Minimal Spanning Sets]
	Let $V$ be a linear space over a field $\mathbb{k}$.
	A spanning set $S \subseteq V$ is a \emph{minimal spanning set} of $V$ if no proper subset of $S$ is a spanning set of $V$, i.e., $S' \subset S \implies \Span(S') \subset \Span(S) = V$ where $\Span(S') \neq V$.
\end{definition}
...
\begin{proposition}
	The order set $S := \{\vec{v}_1, \vec{v}_2, \cdots, \vec{v}_n\} \subseteq V$ is linearly independent if and only if the linear map $\phi_S: \mathbb{k}^n \to V$ defined above is injective.
\end{proposition}

\begin{proposition}
	The order set $S := \{\vec{v}_1, \vec{v}_2, \cdots, \vec{v}_n\} \subseteq V$ is a spanning set of $V$ if and only if the linear map $\phi_S: \mathbb{k}^n \to V$ defined above is surjective.
\end{proposition}

\begin{proposition} \label{prop:minimal_spanning_set}
	The order set $S := \{\vec{v}_1, \vec{v}_2, \cdots, \vec{v}_n\} \subseteq V$ is a minimal spanning set of $V$ if and only if the linear map $\phi_S: \mathbb{k}^n \to V$ defined above is bijective.
\end{proposition}
\begin{remark}
...
Recall the definition of linear independence (Definition~\ref{def:linear_independence}): $V_1$ and $V_2$ are \emph{linearly independent} if $V_1$ and $V_2$ are non-trivial and $x_1 + x_2 = 0$ for $x_i \in V_i$ implies that $x_1 = x_2 = 0$.

We have the following definition for weakly linear independence.
\begin{definition}[Weak Linear Independence]
	Let $V_1$ and $V_2$ be two subspaces of a linear space $W$ over a field $\mathbb{k}$. $V_1$ and $V_2$ are \emph{weakly linearly independent} if $x_1 + x_2 = 0$ for $x_1 \in V_1$ and $x_2 \in V_2$ implies that $x_1 = x_2 = 0$.
...
\begin{proof}
	...
	\begin{center}
		...
		\end{tikzcd}
	\end{center}
	Then the map $\phi : \mathbb{k}^n \to U$ is defined by $x = \alpha^1 \vec{e}_1 + \cdots + \alpha^n \vec{e}_n \mapsto \phi(x) = \alpha^1 \phi(\vec{e}_1) + \cdots + \alpha^n \phi(\vec{e}_n)$.
...
\begin{definition}[Exact and Exact Sequence]
	...
	The sequence is an \emph{exact sequence} if it is exact at every $V_i$.
\end{definition}
...
\begin{definition}[Column Space]
	The \emph{column space} of $A$, denoted by $\col(A)$, is defined as the image of the linear map $f : \mathbb{k}^n \to \mathbb{k}^m$ defined by $f(x) = Ax$, i.e.,
	\[
...
Via the natural identification of $\delta_X \equiv X$ ($\delta_x \equiv x$), an element $\sum \alpha_x \delta_x \in F[X]$, where the sum is finite and $\alpha_x \in \mathbb{k}$, is naturally identified with $\sum \alpha_x x$, which is called a \emph{formal linear combination} of elements in $X$.
Hereafter, we always use this natural identification, so $\mathbb{k}[X]$ is now defined as \emph{the set of formal linear combinations of elements in the set $X$}.
