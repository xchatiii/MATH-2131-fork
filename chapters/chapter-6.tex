...
\begin{definition}[$k$-form]
	...
	\[
		\underbrace{V \times V \times \cdots \times V}_{k \text{ times}} \to \mathbb{k}
	\]
...
\begin{center}
	\begin{tikzcd}
		\Map^{\mathsf{ML}} (V \times V, \mathbb{k}) \arrow[r, "\equiv" description, phantom] \arrow[d, dashed] & \Hom(V, V^*) \arrow[d, "\vequiv" description, phantom] \\
		V^* \otimes V^* & \Hom(V, \mathbb{k}) \otimes V^* \arrow[l, "\equiv" description, phantom]
	\end{tikzcd}
\end{center}
...
\begin{definition}[Symmetric and Skew-symmetric 2-forms]
	A 2-form $\omega : V \times V \to \mathbb{k}$ is called \emph{symmetric}\index{symmetric} if
	\[
		\omega (u, v) = \omega (v, u)
	\]
	for all $u, v \in V$.
...
For $\mathcal{T}^{0, 0} V$, it is defined as $\mathbb{k}$ itself.
...
\begin{center}
	\begin{tikzcd}
		...
		\mathbb{k}^n \arrow[r, "{A = [a^i_j]_{\B_V}}" swap] & \mathbb{k}^n
	\end{tikzcd}
\end{center}
...
An object is considered as covariant if it transforms in the same way as the basis vectors of the original vector space.
...
\]
\begin{remark}
	For easier memorisation, we use calculus operators:
	\[
...
Then we claim that $W$ inherits a Euclidean structure from $\langle -, - \rangle$ in $V$.
...
The complex conjugate vector space $\overline{V}$ is defined as the same set as $V$ with the same addition operation, but the scalar multiplication is defined as:
\[
