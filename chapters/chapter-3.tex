...
\begin{definition}[Linear Subspaces]
	Let $W$ be a linear space over $\mathbb{k}$ and $V$ is a subset of $W$, denoted as $V \subset W$. $V$ is a \emph{linear subspace}\index{linear subspace} of $W$ if $V$, with $+$ and $\cdot$ inherited from those of $W$, is a linear space.
\end{definition}
...
\begin{proof}
	...
	If $V$ is not empty and closed under $+$ and $\cdot$, we just have to check each axiom.
\end{proof}
...
\begin{example}
	...
	Then we consider $\alpha^1, \alpha^2 \in \mathbb{k}$ and $v_1, v_2 \in \ker f$, we have
	\[
...
\begin{example}
	...
	Then we consider $\alpha^1, \alpha^2 \in \mathbb{k}$ and $f(v_1), f(v_2) \in \im f$.
	We have
	\[
...
\begin{example}
	Let $W$ be a linear space over a field $\mathbb{k}$ and $\{V_\alpha\}_{\alpha \in I}$ be the family of subspaces of $W$ indexed by the element in the index set $I$.
	...
	Then we consider $\alpha^1, \alpha^2 \in \mathbb{k}$ and $v_1, v_2 \in \bigcap_{\alpha \in I} V_\alpha$.
...
\begin{proposition}
	Let $W$ be a linear space over a field $\mathbb{k}$ and consider the family of subspaces $\{V_\alpha\}_{\alpha \in I}$.
...
\begin{definition}[Linear Span]
	Let $V$ be a linear space over a field $\mathbb{k}$ and $S \subset V$.
	The \emph{linear span}\index{linear span} of $S$, denoted by $\Span_\mathbb{k} (S)$ or simply $\Span S$ or $\bar{S}$ or $\langle S \rangle$, is defined as the completion of $S$ inside $V$ under linear combinations.
\end{definition}
...
\begin{proposition}
	Let $W$ be a linear space over a field $\mathbb{k}$ and $S \subset W$.
	Then
	\[
		\Span S = \left\{ \sum_{i=1}^n \alpha^i s_i \mid n \in \mathbb{N}, \alpha^i \in \mathbb{k}, s_i \in S \right\}
	\]
	Note that the summation is a finite summation.
\end{proposition}

\begin{definition}[Linear Independences] \label{def:linear_independence}
	Let $W$ be a linear space over a field $\mathbb{k}$ and $V_1, \cdots, V_k$ be subspaces of $W$.
	The subspaces $V_1, \cdots, V_k$ are said to be \emph{linearly independent}\index{linearly independent} if $V_i \neq \{0_W\}$ for all $i$ and there is one and only one way to split $0_W \in W$ as a sum of vectors from each $V_i$, i.e., if $v_i \in V_i$ for all $i$ and $\sum_{i=1}^k v_i = 0_W$, then $v_i = 0_W$ for all $i$.
\end{definition}

Vectors $v_1, v_2, \cdots, v_k \in W$ are independent if the subspaces $\Span(v_1)$, $\Span(v_2)$, $\cdots$, $\Span(v_k)$ are linearly independent.

\begin{proposition}
	$v_1, v_2, \cdots, v_k \in W$ are linearly independent if and only if there is one and only one way to write $0_W \in W$ as the combination of $v_1, \cdots, v_k$ with coefficients in $\mathbb{k}$, i.e., the equation
	\[
...
\begin{definition}[Linearly Independent Sets]
	Let $V$ be a linear space over a field $\mathbb{k}$.
	A subset $S \subseteq V$ is said to be a \emph{linearly independent set}\index{linearly independent set} of vectors in $V$ if no elements in $S$ can be expressed as a linear combination of the finitely many other elements in $S$.
\end{definition}

\begin{definition}[Spanning Sets]
	Let $V$ be a linear space over a field $\mathbb{k}$.
	A subset $S \subseteq V$ is said to be a \emph{spanning set}\index{spanning set} of $V$ if $\Span(S) = V$.
\end{definition}

\begin{example}
	Let $V = \mathbb{k}^3$ and consider the three vectors $\vec e_1$, $\vec e_2$ and $\vec e_3$.

	Then the set $S = \{\vec e_1, \vec e_2, \vec e_1 + \vec e_2\}$ is not a spanning set of $V$ as $\Span(S) = \Span \{\vec e_1, \vec e_2\} \neq V$.
	If we consider the $\Span \{\vec e_1, \vec e_2\} = W$, then $\{\vec e_1, \vec e_2\}$ is a minimal spanning set of $W$.

	The set $S = \{\vec e_1, \vec e_1 + \vec e_2, \vec e_1 + \vec e_2 + \vec e_3\}$ is a spanning set of $V$.
\end{example}
\begin{remark}
	If we consider the matrix of $\{\vec e_1, \vec e_2, \vec e_1 + \vec e_2\}$ with respect to the standard basis of $\mathbb{k}^3$, we have:
	\[
		A = \begin{bmatrix}
			1 & 0 & 1 \\
			0 & 1 & 1 \\
	...
	Thus, the set is not a spanning set of $\mathbb{k}^3$.
\end{remark}

\begin{example}
	Consider the subset $S = \{ 1, t, t^2, \cdots \} \subset \mathbb{k}[[t]]$.
	Then $\Span(S) = \mathbb{k}[t]$ which is a proper subspace of $\mathbb{k}[[t]]$.
	As the linear combination of finitely many elements in $S$ is a polynomial, but an element in $\mathbb{k}[[t]]$ can be a power series.
\end{example}

\begin{definition}[Minimal Spanning Sets]
	Let $V$ be a linear space over a field $\mathbb{k}$.
	A spanning set $S \subseteq V$ is said to be a \emph{minimal spanning set}\index{minimal spanning set} of $V$ if no proper subset of $S$ is a spanning set of $V$, i.e., $S' \subset S \implies \Span(S') \subset \Span(S) = V$ where $\Span(S') \neq V$.
\end{definition}

The following is also the equivalence definition of linearly independent sets, spanning sets and minimal spanning sets.

Given a linear space $V$ over a field $\mathbb{k}$.
We define the order set $S := \{\vec v_1, \vec v_2, \cdots, \vec v_n\} \subseteq V$.
The order set $S$ forms a linear map $\phi_S: \mathbb{k}^n \to V$ defined by:
\[
	\phi_S(\vec x) = \phi_S\left(\begin{bmatrix}
		...
		\vdots \\
		x^n
	\end{bmatrix}\right) = x^1 \vec v_1 + x^2 \vec v_2 + \cdots + x^n \vec v_n = \sum_{i=1}^n x^i \vec v_i
\]

\begin{proposition}
	The order set $S := \{\vec v_1, \vec v_2, \cdots, \vec v_n\} \subseteq V$ is said to be linearly independent if and only if the linear map $\phi_S: \mathbb{k}^n \to V$ defined above is injective.
\end{proposition}

\begin{proposition}
	The order set $S := \{\vec v_1, \vec v_2, \cdots, \vec v_n\} \subseteq V$ is said to be a spanning set of $V$ if and only if the linear map $\phi_S: \mathbb{k}^n \to V$ defined above is surjective.
\end{proposition}

\begin{proposition} \label{prop:minimal_spanning_set}
	The order set $S := \{\vec v_1, \vec v_2, \cdots, \vec v_n\} \subseteq V$ is said to be a minimal spanning set of $V$ if and only if the linear map $\phi_S: \mathbb{k}^n \to V$ defined above is bijective.
\end{proposition}
\begin{remark}
	A minimal spanning set is regarded as \emph{basis}\index{basis}.
\end{remark}

\begin{example}
	Let $X$ be a set, $\mathbb{k}[[X]]$ be the set of all functions $f: X \to \mathbb{k}$ and $\mathbb{k}[X]$ be the set of all finite support functions $f: X \to \mathbb{k}$.
	For each $x \in X$, we define the Kronecker delta function $\delta_x : X \to \mathbb{k}$ at point $x$ by
	\[
		...
		\end{cases}
	\]
	Clearly, $\delta_x$ has finite support, thus $\delta_x \in \mathbb{k}[X]$.

	Then we have a set $\delta_X = \{\delta_x \mid x \in X\} \subset \mathbb{k}[X]$.
	We have $\Span(\delta_X) = \mathbb{k}[X]$ as any finite support function $f: X \to \mathbb{k}$ can be written as a linear combination of finitely many delta functions.
	Thus, $\delta_X$ is a spanning set of $\mathbb{k}[X]$.
...
\begin{definition}[Group Actions]
	...
	A \emph{left group action}\index{left group action} of $G$ on $X$ is a map $\cdot : G \times X \to X$, $(g, x) \mapsto g \cdot x$, such that for all $g_1, g_2 \in G$ and $x \in X$, the following properties hold:
	\begin{enumerate}
...
Let $V$ be a subspace of a linear space $W$ over a field $\mathbb{k}$.
...
\begin{definition}[Quotient Spaces]
	Let $W$ be a linear space over a field $\mathbb{k}$ and $V$ be a subspace of $W$.
	The \emph{quotient space}\index{quotient space} of $W$ by $V$, denoted by $\quotient{W}{V}$, is defined as the set of orbits of the group action of $V$ on $W$, or the set of $V$-equivalence classes in $W$ with the equivalence relation defined above, i.e.,
	\[
...
\begin{definition}[Quotient Map]
	The natural surjective map $\pi : W \to \quotient{W}{V}$ defined by $\pi(w) = w + V$ for all $w \in W$ is called the \emph{quotient map}\index{quotient map} or \emph{projection map}\index{projection map}.
...
\begin{proof}
	...
	Then for all $w_1, w_2 \in W$ and $\alpha_1, \alpha_2 \in \mathbb{k}$, we have
	\[
...
\begin{proposition}
	Let $V$ be a linear space over a field $\mathbb{k}$ and $S$ be a minimal spanning set of $V$.
	Then for any set map $\phi : S \to Z$, where $Z$ is any linear space over $\mathbb{k}$, there is a unique linear map $\tilde{\phi} : V \to Z$ such that $\tilde{\phi}|_S = \phi$.
...
\begin{proof}
	...
	Since $S$ is a minimal spanning set of $V$, for any $v \in V$, we have a unique way to write $v$ as a linear combination of finitely many elements in $S$, i.e., $v = \sum_{i=1}^n \alpha_i s_i$ where $\alpha_i \in \mathbb{k}$ and $s_i \in S$ are distinct.
	Then we have
	\[
...
\begin{proposition}
	...
	\begin{center}
		...
		\end{tikzcd}
	\end{center}
	where $Z$ is any linear space over $\mathbb{k}$ and $\phi : W \to Z$ is any linear map such that $\phi(v) = 0_Z$ for all $v \in V$.
...
\begin{proof}
	...
	For all $[w_1], [w_2] \in \quotient{W}{V}$ and $\alpha^1, \alpha^2 \in \mathbb{k}$, we have
	\begin{align*}
...
\begin{definition}[Cokernel]
	Let $f : V \to W$ be a linear map between two linear spaces over a field $\mathbb{k}$.
...
\begin{definition}[Coimage]
	Let $f : W \to V$ be a linear map between two linear spaces over a field $\mathbb{k}$.
...
\begin{definition}[Sum of Subspaces]
	Let $V_1$ and $V_2$ be two subspaces of a linear space $W$ over a field $\mathbb{k}$.
...
\begin{proposition}
	The sum $V_1 + V_2$ of two subspaces $V_1$ and $V_2$ of a linear space $W$ over a field $\mathbb{k}$ is also a subspace of $W$.
\end{proposition}
...
Recall the definition of linear independence (Definition~\ref{def:linear_independence}): $V_1$ and $V_2$ are said to be \emph{linearly independent}\index{linearly independent} if $V_1$ and $V_2$ are non-trivial and $x_1 + x_2 = 0$ for $x_i \in V_i$ implies that $x_1 = x_2 = 0$.

We have the following definition for weakly linear independence.
\begin{definition}[Weak Linear Independence]
	Let $V_1$ and $V_2$ be two subspaces of a linear space $W$ over a field $\mathbb{k}$. $V_1$ and $V_2$ are said to be \emph{weakly linearly independent}\index{weakly linearly independent} if $x_1 + x_2 = 0$ for $x_1 \in V_1$ and $x_2 \in V_2$ implies that $x_1 = x_2 = 0$.
...
\begin{definition}[Direct Sum of Subspaces]
	Let $V_1$ and $V_2$ be two subspaces of a linear space $W$ over a field $\mathbb{k}$.
	The \emph{direct sum}\index{direct sum} of $V_1$ and $V_2$, denoted by $V_1 \oplus V_2$, is defined as the sum $V_1 + V_2$ when $V_1$ and $V_2$ are weakly linearly independent, i.e.,
	\[
...
Recall (Definition~\ref{def:finite_dimensional_vector_space}) that $W$ is a finite dimensional if $W \cong \mathbb{k}^n$ for some positive integer $n$.
It is equivalent to saying that $W$ is finitely spanned, i.e., having a finite spanning set.

\begin{proof}
	If we have a map $\phi : \mathbb{k}^n \to W$, then $W = \Span\{\phi(e_1), \phi(e_2), \cdots, \phi(e_n)\}$.
	...
	Then using (Proposition~\ref{prop:minimal_spanning_set}), we have a bijective map $\phi_{\{e_1, e_2, \cdots, e_k\}} : \mathbb{k}^k \to W = \Span\{\phi(e_1), \phi(e_2), \cdots, \phi(e_k)\}$.
\end{proof}
...
\begin{proof}
	...
	\begin{center}
		\begin{tikzcd}
			...
			\mathbb{k}^n \arrow[u, hook, two heads] \arrow[ur, two heads, dashed, "\phi"]
		\end{tikzcd}
	\end{center}
	Then the map $\phi : \mathbb{k}^n \to U$ is defined by $x = \alpha^1 \vec e_1 + \cdots + \alpha^n \vec e_n \mapsto \phi(x) = \alpha^1 \phi(\vec e_1) + \cdots + \alpha^n \phi(\vec e_n)$.
	Thus, $U$ is finitely spanned, $U = \Span\{\phi(\vec e_1), \phi(\vec e_2), \cdots, \phi(\vec e_n)\}$.
	...
	\begin{center}
		...
		\end{tikzcd}
	\end{center}
	Then we know that $\pi(\vec e_1), \pi(\vec e_2), \cdots, \pi(\vec e_n)$ spans $\quotient{W}{V}$.
...
\begin{proof}
	...
	Then we can write $v_i$ as a linear combination of finitely many elements in $S_i$, i.e., $v_i = \sum_{j=1}^{n_i} \alpha_i^j s_i^j$ where $\alpha_i^j \in \mathbb{k}$ and $s_i^j \in S_i$ are distinct.
	Thus, we have
	\[
...
\begin{definition}[Exact and Exact Sequence]
	A sequence of linear maps between linear spaces over a field $\mathbb{k}$,
	\begin{center}
		\begin{tikzcd}
			\cdots \arrow[r, "f_{i-2}"] & V_{i-1} \arrow[r, "f_{i-1}"] & V_i \arrow[r, "f_i"] & V_{i+1} \arrow[r, "f_{i+1}"] & \cdots
		\end{tikzcd}
	\end{center}
	is \emph{exact}\index{exact} at $V_i$ if
	\[ \im{f_{i-1}} = \ker(f_i) \]
	i.e., the image of the map before $V_i$ is equal to the kernel of the map after $V_i$.

	The sequence is said to be an \emph{exact sequence}\index{exact sequence} if it is exact at every $V_i$.
\end{definition}
...
\begin{center}
	\begin{tikzcd}
		\ker(f) \subseteq \mathbb{k}^n \arrow[r, "f", "A" swap] & \mathbb{k}^m \supseteq \im{f}
	\end{tikzcd}
\end{center}

In normal linear algebra, we have four fundamental concepts: column space, null space, rank and nullity.
\begin{definition}[Column Space]
	The \emph{column space}\index{column space} of $A$, denoted by $\col(A)$, is defined as the image of the linear map $f : \mathbb{k}^n \to \mathbb{k}^m$ defined by $f(x) = Ax$, i.e.,
	\[
		\col(A) = \im{f} = \{ Ax \mid x \in \mathbb{k}^n \} \subseteq \mathbb{k}^m
	\]
\end{definition}
...
\begin{definition}[Null Space]
	The \emph{null space}\index{null space} of $A$, denoted by $\nul(A)$, is defined as the kernel of the linear map $f : \mathbb{k}^n \to \mathbb{k}^m$ defined by $f(x) = Ax$, i.e.,
	\[
		\nul(A) = \ker(f) = \{ x \in \mathbb{k}^n \mid Ax = 0 \} \subseteq \mathbb{k}^n
	\]
\end{definition}
...
\begin{theorem}[Rank-Nullity Theorem]
	For a linear map $f : V \to W$ between finite dimensional linear spaces over $\mathbb{k}$, we have
	\[
		\rank(f) + \nullity(f) = \dim{V}
	\]
\end{theorem}
...
First, let $f : V_1 \to V_2$ be a linear map between finite dimensional linear spaces over $\mathbb{k}$.
...
$s_1$ and $s_2$ are the \emph{right inverses}\index{right inverses} or called \emph{sections}\index{sections}.
...
\begin{center}
	\begin{tikzpicture}
		\matrix (m) [matrix of math nodes, column sep=4.8em, minimum width=2em]
		{\mathbb{k}^r \oplus \mathbb{k}^{n - r} & \mathbb{k}^r \oplus \mathbb{k}^{n - r} \\};
...
Here $\delta_x : X \to \mathbb{k}$ is the $\delta$-function at $x$.

\begin{proposition}
	$\delta_X$ is a linearly independent set of $\mathbb{k}[[X]] =$ the linear space of $\mathbb{k}$-valued functions on $X$.
\end{proposition}

\begin{proposition}
	$\Span(\delta_X) = \mathbb{k}[X]$
\end{proposition}

Then $\delta_X$ is a minimal spanning set for $\mathbb{k}[X]$.

\begin{proposition}
	There is a natural set isomorphism $X \to \delta_X$ which maps $x$ to $\delta_x$.
\end{proposition}

Then we have an injective set map $\iota : X \equiv \delta_X \to \mathbb{k}[X]$ which maps $x$ to $\delta_x$.
This is a set mapping to a linear space.

Among all set maps from $X$ to a linear space over $\mathbb{k}$, the set map $\iota : X \to \mathbb{k}[X]$ is universal in the following sense:
\begin{center}
	\begin{tikzcd}
		X \arrow[r, "\forall \phi"] \arrow[d, "\iota"] & Z \\
		\mathbb{k}[X] \arrow[ur, "\exists ! \tilde{\phi}" swap]
	\end{tikzcd}
\end{center}
For any set map $\phi : X \to Z$, there exists a unique linear map $\tilde{\phi} : \mathbb{k}[X] \to Z$ such that $\tilde{\phi} \circ \iota = \phi$.
...
\begin{proof}
	...
	As $\{ \delta_x \mid x \in X \}$ is a minimal spanning set for $\mathbb{k}[X]$, $\tilde{\phi}$ must be the linear map such that $\tilde{\phi} (\delta_x) = \phi(x)$, thus unique.
	Existence of $\tilde{\phi}$ is also proved.
\end{proof}

Via the natural identification of $\delta_X \equiv X$ ($\delta_x \equiv x$), an element $\sum \alpha_x \delta_x \in F[X]$, where the sum is finite and $\alpha_x \in \mathbb{k}$, is naturally identified with $\sum \alpha_x x$, which is called a \emph{formal linear combination}\index{formal linear combination} of elements in $X$.
Hereafter, we always use this natural identification, so $\mathbb{k}[X]$ is now defined as \emph{the set of formal linear combinations of elements in the set $X$}\index{the set of formal linear combinations of elements in the set $X$}.
Then $\iota : X \to \mathbb{k}[X]$ is just the inclusion map : $x \mapsto x$.

The universal map is unique in the following sense: suppose that $\iota' : X \to \mathbb{k}[X]'$ is another inclusion map, then there is a unique linear equivalence $\lambda$ in the commutative triangle:
\begin{center}
	\begin{tikzcd}[column sep=normal]
		& X \arrow[dl, "\iota" swap] \arrow[dr, "\iota'"] \\
		\mathbb{k}[X] \arrow[rr, "\lambda" swap, hook, two heads] & & \mathbb{k}[X]'
	\end{tikzcd}
\end{center}
...
\begin{center}
	\begin{tikzcd}[column sep=normal]
		...
		\mathbb{k}[X]' \arrow[r, "\mu" swap, dashed] \arrow[rrr, bend right, "1"] & \mathbb{k}[X] \arrow[rr, "\lambda" swap, dashed] \arrow[rrr, bend right, "1"] & & \mathbb{k}[X]' \arrow[r, "\mu" swap, dashed] & \mathbb{k}[X]
	\end{tikzcd}
\end{center}
...
The universal property implies an assignment of a linear map $\mathbb{k}[f] : \mathbb{k}[X] \to \mathbb{k}[Y]$ to any set map $f : X \to Y$.
Indeed,
\begin{center}
	\begin{tikzcd}
		X \arrow[r, "f"] \arrow[d, "\iota" swap, hook] \arrow[dr, "\iota f"] & Y \arrow[d, "\iota", hook] \\
		\mathbb{k}[X] \arrow[r, dashed, "{\exists ! \mathbb{k}[f]}" swap] & \mathbb{k}[Y]
	\end{tikzcd}
\end{center}

Moreover, $\mathbb{k}[1_X] = 1_{\mathbb{k}[X]}$ or simply $\mathbb{k}[1] = 1$ for all $X$, and $\mathbb{k}[fg] = \mathbb{k}[f] \mathbb{k}[g]$ for all $f : Y \to Z$ and $g : X \to Y$.
