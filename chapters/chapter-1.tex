...
\begin{definition}[Unital]
	A binary operation $\cdot: S \times S \to S$ is \emph{unital}\index{unital} if there exists an element $e \in S$, the \emph{identity}\index{identity} or \emph{unit element}, such that, for all $x \in S$,
	\[ e \cdot x = x = x \cdot e \]
\end{definition}
...
\begin{definition}[Group Homomorphism]
	...
	A function $f: G_1 \to G_2$ is a \emph{group homomorphism} if:
	\begin{enumerate}
