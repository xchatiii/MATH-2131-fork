...
Linear map, sometimes linear transformation, is a homomorphism that preserves linear structure.

\begin{definition}[Linear Maps]
	Let $V$ and $W$ be two linear spaces over a field $\mathbb{k}$.
	A \emph{linear map}\index{linear map} is a set map $T: V \to W$ such that for all $u, v \in V$ and $\alpha \in \mathbb{k}$, the following holds:
	\begin{align*}
		T(u + v) &= T(u) + T(v) \\
		T(\alpha u) &= \alpha T(u)
	\end{align*}
...
\begin{definition}[Linear Combinations]
	...
	A \emph{linear combination}\index{linear combination} of vectors $v_1, v_2, \cdots, v_n \in V$ is a vector of the form:
	\[
		\alpha^1 v_1 + \alpha^2 v_2 + \cdots + \alpha^n v_n
	\]
	where $\alpha^1, \alpha^2, \cdots, \alpha^n \in \mathbb{k}$ are scalars.
\end{definition}
...
\begin{corollary}[Linear Maps and Linear Combinations]
	A set map $f: V \to W$ between two linear spaces over a field $\mathbb{k}$ is a linear map if and only if $T$ respects linear combinations, i.e., for all $v_1, v_2 \in V$ and all scalars $\alpha^1, \alpha^2 \in \mathbb{k}$, the following holds:
	\[
...
\begin{example}
	Let $A$ be an $m \times n$ matrix with entries in a field $\mathbb{k}$.
	The map $T: \mathbb{k}^n \to \mathbb{k}^m$ defined by
	\[
		Tx = T(x) = Ax
	\]
	where right-hand side is the usual matrix multiplication, is a linear map over $\mathbb{k}$.
\end{example}

\begin{proposition}
	A linear map $T: \mathbb{k}^n \to \mathbb{k}^m$ is a matrix multiplication by a unique $m \times n$ matrix $A$ with entries in $\mathbb{k}$.
	The matrix $A$ is called the \emph{standard matrix}\index{standard matrix} of the linear map $T$.

	\begin{center}
		\begin{tikzcd}[column sep=huge, row sep=tiny]
			\Hom(\mathbb{k}^n, \mathbb{k}^m) \arrow[r, equal, "\text{natural}", "\text{identification}" swap] & \M{m \times n}{\mathbb{k}} \\
			T \arrow[r, mapsto] & A \\
...
\begin{proof}
	Consider a column matrix $x \in \mathbb{k}^n$ with entries $x^1, x^2, \cdots, x^n \in \mathbb{k}$.
	Then $x$ can be expressed as a linear combination of the vectors $\vec e_1, \vec e_2, \cdots, \vec e_n$:
	\[
		x = x^1 \vec e_1 + x^2 \vec e_2 + \cdots + x^n \vec e_n = \sum_{i=1}^n x^i \vec e_i
	\]
	Since $T$ is a linear map, it respects linear combinations.
	Therefore, we have:
	\[
		Tx = T\left( \sum_{i=1}^n x^i \vec e_i \right) = \sum_{i=1}^n x^i T(\vec e_i) = \sum_{i=1}^n x^i \vec a_i = A\vec x
	\]
	where $\vec a_i = T\vec e_i$ is the $i$-th column of the matrix $A = \begin{bmatrix}
		...
		| & | & & |
	\end{bmatrix}$.
	Thus, we have $T\vec x = A\vec x$ for all $\vec x \in \mathbb{k}^n$.
	This shows that $T$ can be represented as a matrix multiplication by the matrix $A$.
\end{proof}

There is a simpler way to write $\sum_{i=1}^n x^i \vec e_i$: The Einstein Summation Convention.
...
\begin{definition}[Linear Functional / Homogeneous Linear Function]
	A linear map $f: \mathbb{k}^n \to \mathbb{k}$ is called a \emph{homogeneous linear function}\index{homogeneous linear function} or a \emph{linear functional}\index{linear functional} if for all $\alpha \in \mathbb{k}$ and $x \in \mathbb{k}^n$, the following holds:
	\[
		f(\alpha x) = \alpha f(x)
	\]
\end{definition}
...
\begin{corollary}[Standard Matrix of a Linear Map]
	The standard matrix of a linear map $T: \mathbb{k}^n \to \mathbb{k}^m$ can be written as:
	\[
		A = \begin{bmatrix}
			\hdash & f_1 & \hdash \\
			\hdash & f_2 & \hdash \\
		...
		\end{bmatrix}
	\]
	where $f_i: \mathbb{k}^n \to \mathbb{k}$ is the $i$-th component function of $T$, which is a linear functional.
\end{corollary}

\begin{example}
	Let $D: \mathbb{k}[t] \to \mathbb{k}[t]$ be the differentiation operator defined by:
	\[
		D\left( \sum_{n=0}^N a_n t^n \right) = \sum_{n=1}^N n a_n t^{n-1}
	\]
	for all polynomials $\sum_{n=0}^N a_n t^n \in \mathbb{k}[t]$.
	The differentiation operator $D$ is a linear map over $\mathbb{k}$.
	The standard matrix of $D$ with respect to the standard basis $\{1, t, t^2, \cdots, t^N\}$ of $\mathbb{k}[t]$ is given by:
	\[
		A = \begin{bmatrix}
			0 & 1 & 0 & 0 & \cdots & 0 \\
			0 & 0 & 2 & 0 & \cdots & 0 \\
...
\begin{proposition}
	Let $X$ be a set and $W$ be a linear space over a field $\mathbb{k}$.
	Then the set of all set maps from $X$ to $W$, denoted by $\Map(X, W)$, is a linear space over $\mathbb{k}$ with the following operations defined pointwise:
	\begin{align*}
		+ : \Map(X, W) \times \Map(X, W) &\to \Map(X, W) \\
		(f,g) &\mapsto (f + g) : x \mapsto f(x) + g(x) \\
		\cdot : \mathbb{k} \times \Map(X, W) &\to \Map(X, W) \\
		(\alpha,f) &\mapsto (\alpha f) : x \mapsto \alpha f(x)
	\end{align*}
\end{proposition}

\begin{proof}
	The $\Map(X, W)$ is defined pointwise by $\mathbb{k}$, hence it is trivial to be a linear map.
\end{proof}

\begin{proposition}
	Let $V$ and $W$ be two linear spaces over a field $\mathbb{k}$.
	Then $\Hom(V, W)$ is a linear space over $\mathbb{k}$ with the following operations defined pointwise:
	\begin{align*}
		+ : \Hom(V, W) \times \Hom(V, W) &\to \Hom(V, W) \\
		(f,g) &\mapsto (f + g): v \mapsto f(v) + g(v) \\
		\cdot : \mathbb{k} \times \Hom(V, W) &\to \Hom(V, W) \\
...
\begin{proof}
	...
	We need to show that the operations defined above are closed in $\Hom(V, W)$, i.e., for all $f, g \in \Hom(V, W)$ and $\alpha \in \mathbb{k}$, $f + g \in \Hom(V, W)$ and $\alpha f \in \Hom(V, W)$ or equivalently, $f$ respects linear combinations.

	Let $\vec u, \vec v \in V$ and $\alpha, \beta \in \mathbb{k}$.
...
\end{proof}
\begin{remark}
	Note that $\End(V) = \Hom(V, V)$ is a linear space over $\mathbb{k}$ and also a ring with the addition and multiplication operations defined in the previous section.
	The addition operation is commutative, but the multiplication operation is not necessarily commutative.
\end{remark}

Then we can say that
\[ \Map(\mathbb{k}^n, \mathbb{k}^m) \supseteq \Hom(\mathbb{k}^n, \mathbb{k}^m) \cong \M{m \times n}{\mathbb{k}} \]
...
\begin{definition}[Injective Lienar Maps]
	A linear map $f: V \to W$ between two linear spaces over a field $\mathbb{k}$ is \emph{injective}\index{injective} (or one-to-one) if for all $u, v \in V$, the following holds:
	\[
		f(u) = f(v) \implies u = v
	\]
...
\begin{definition}[Surjective Lienar Maps]
	A linear map $f: V \to W$ is \emph{surjective}\index{surjective} (or onto) if for every $w \in W$, there exists at least one $v \in V$ such that:
	\[
		w = f(v)
	\]
\end{definition}
...
\begin{definition}[Invertible Linear Maps / Linear Equivalences]
	A linear map $T: V \to W$ is \emph{invertible}\index{invertible} if $T$ has a unique two-sided inverse $S$, denoted by $T^{-1}$, i.e., there exists a linear map $S: W \to V$ such that:
	\[
		TS = 1_W \quad \text{and} \quad ST = 1_V
	\]
	where $1_V: V \to V$ and $1_W: W \to W$ are the identity maps on $V$ and $W$, respectively.
	In this case, we say that the linear spaces $V$ and $W$ are \emph{isomorphic}\index{isomorphic} or \emph{linear equivalent}\index{linear equivalent}, denoted by $V \cong W$.
\end{definition}
...
\begin{definition}[Characteristic of a Field]
	The \emph{characteristic}\index{characteristic} of a field $\mathbb{k}$ is the smallest positive integer $n$ such that:
	\[
		\underbrace{1 + 1 + \cdots + 1}_{n \text{ times}} = 0
	\]
	If no such positive integer exists, the characteristic of $\mathbb{k}$ is defined to be $0$.
\end{definition}

\begin{example}
	The differentiation operator $D: \mathbb{k}[t] \to \mathbb{k}[t]$ is not an injective linear map as $D(1) = 0 = D(2)$ but is a surjective linear map if $\mathbb{k}$ is a field of characteristic $0$.
\end{example}
...
We consider two linear maps $T: \mathbb{k}^n \to \mathbb{k}^m$ and $S: \mathbb{k}^m \to \mathbb{k}^k$ with standard matrices $A$ and $B$, respectively.
We want to find the standard matrix of the composition $ST: \mathbb{k}^n \to \mathbb{k}^k$.

\begin{center}
	\begin{tikzpicture}
		\matrix (m) [matrix of math nodes, row sep=3em, column sep=4em, minimum width=2em]
		{\mathbb{k}^n & \mathbb{k}^m & \mathbb{k}^k \\};
		\path[->]
...
\begin{proposition}
	The standard matrix of the composition $ST: \mathbb{k}^n \to \mathbb{k}^k$ is the matrix multiplication $BA$, i.e., for all $x \in \mathbb{k}^n$,
	\[
		(ST)x = B(Ax) = (BA)x
	\]
\end{proposition}
...
\begin{proof}
	Let $x \in \mathbb{k}^n$ be a column matrix with entries $x^1, x^2, \cdots, x^n \in \mathbb{k}$.
	Then $x$ can be expressed as a linear combination of the standard basis vectors $\vec e_1, \vec e_2, \cdots, \vec e_n$:
	\[
		x = x^1 \vec e_1 + x^2 \vec e_2 + \cdots + x^n \vec e_n = x^i \vec e_i
	\]
...
\[
	...
	\end{bmatrix}
\]
where $\vec a_i = T(\vec e_i)$ is the $i$-th column of the matrix $A$.
Also,
\[
	Bx = x^1 \vec b_1 + x^2 \vec b_2 + \cdots + x^n \vec b_n = x^i \vec b_i
\]
where $\vec b_i = B\vec a_i$ is the $i$-th column of the matrix $B$.
Note that $B$ is a $k \times m$ matrix, and $x \in \mathbb{k}^m$.
Thus, the matrix multiplication $Bx$ is defined and results in a column matrix in $\mathbb{k}^k$.
...
\begin{definition}[Elementary Row Operations]
	Let $A$ be an $m \times n$ matrix over a field $\mathbb{k}$.
	An \emph{elementary row operation}\index{elementary row operation} on $A$ is one of the following operations:
	\begin{enumerate}
		\item Row Interchange: \qquad $R_i \leftrightarrow R_j$.
		\item Row Multiplication: \quad $R_i \to \alpha R_i$, where $\alpha \in \mathbb{k} \setminus \{0\}$.
		\item Row Addition: \qquad\quad\ $R_i \to R_i + \alpha R_j$, where $\alpha \in \mathbb{k}$ and $i \neq j$.
	\end{enumerate}
	Each elementary row operation can be represented by \emph{left multiplication}\index{left multiplication} of $A$ by an appropriate $m \times m$ matrix over $\mathbb{k}$.
	Note that all of them are invertible linear maps from $\mathbb{k}^{m \times n}$ to $\mathbb{k}^{m \times n}$.
\end{definition}
...
\begin{definition}[Matrix Units]
	Let $m$ and $n$ be two positive integers and $\mathbb{k}$ be a field.
	The \emph{matrix unit}\index{matrix unit} $E_i^j$ is the $m \times n$ matrix over $\mathbb{k}$ with $1$ in the $(i,j)$-th position and $0$ elsewhere, i.e.,
	\[
	...
	It can also be defined as $E_i^j = \vec e_i \hat e^j \in \M{m \times n}{\mathbb{k}}$ where $\vec e_i \in \mathbb{k}^m$ and $\vec e_j^T = \hat e^j \in (\mathbb{k}^n)^*$ are the $i$-th and $j$-th standard basis vectors, respectively.
	The $\hat e^j$ is the row matrix with 1 in the $j$-th column and 0 anywhere else.
\end{definition}
\begin{remark}
	Note that for any $m \times n$ matrix $A$ over a field $\mathbb{k}$, we have:
	\begin{align*}
		A\vec e_j &= \text{the } j\text{-th column of } A \in \mathbb{k}^n \\
		\hat e^i A &= \text{the } i\text{-th row of } A \in (\mathbb{k}^m)^* \\
	\end{align*}
	where $(\mathbb{k}^m)^*$ is the set of all row matrices with $n$ entries in $\mathbb{k}$. $\hat e^i$ is an element in $(\mathbb{k}^m)^*$ for any $1 \leq i \leq m$.
	Then we have: (Be aware of the difference between superscript and subscript)
	\[
		a_j^i = \hat e^i A \vec e_j = \text{the } (i,j)\text{-th entry of } A
	\]
\end{remark}
...
\vspace{6ex}
\[
	E_i^j = \vec e_i \hat e^j = \begin{bmatrix}
		0 \\
		\vdots \\
		0 \\
		1 \\
...
\begin{proof}
	The linear map $T: \mathbb{k}^n \to \mathbb{k}^n$ is defined pointwise.
	...
	\[
		A_{R_i \leftrightarrow R_j} = \begin{bmatrix}
			| & & | & & | & & | \\
			\vec e_1 & \cdots & \vec e_j & \cdots & \vec e_i & \cdots & \vec e_n \\
			| & & | & & | & & |
...
\begin{proposition}
	The row operation $R_i \to \alpha R_i$ where $\alpha \in \mathbb{k}^\times := \mathbb{k} \setminus \{0\}$ is a linear map where the standard matrix is $A_{R_i \to \alpha R_i} = I + (\alpha - 1) E_i^i$.
\end{proposition}
...
\begin{proof}
	...
	\[
		A_{R_i \to \alpha R_i} = \begin{bmatrix}
			| & & | & & | \\
			\vec e_1 & \cdots & \alpha\vec e_i & \cdots & \vec e_n \\
			| & & | & & |
...
\begin{proof}
	...
	\[
		A_{R_i \to R_i + \alpha R_j} = \begin{bmatrix}
			| & & | & & | & & | \\
			\vec e_1 & \cdots & \vec e_i + \alpha\vec e_j & \cdots & \vec e_n \\
			| & & | & & |
...
\begin{definition}[Finite Dimensional Vector Spaces] \label{def:finite_dimensional_vector_space}
	A linear space $V$ over a field $\mathbb{k}$ is said to be \emph{finite dimensional}\index{finite dimensional} if there exists a linear equivalence $T: V \to \mathbb{k}^n$ for some positive integer $n$.
	In this case, we say that the dimension of $V$ is $n$, denoted $\dim_\mathbb{k} V = n$ or simply $\dim V = n$.
\end{definition}

\begin{definition}[Infinite Dimensional Vector Spaces]
	A linear space $V$ over a field $\mathbb{k}$ is said to be \emph{infinite dimensional}\index{infinite dimensional} if $V$ is not finite dimensional.
\end{definition}

We have to proof if the dimension of a finite dimensional vector space is well-defined.

\begin{proposition}
	If there exists two linear equivalences $T: V \to \mathbb{k}^m$ and $S: V \to \mathbb{k}^n$, then $n = m$.
\end{proposition}

\begin{proof}
	Since $S$ is linear equivalence, it has a unique two-sided inverses $S^{-1}: \mathbb{k}^n \to V$.
	Consider the composition of this map:
	\[
		TS^{-1}: \mathbb{k}^n \to \mathbb{k}^m
	\]
	Since $TS^{-1}$ is compositions of linear equivalences, it is also a linear equivalence.
	Mutantis mutandis, for the opposite direction. % did you do DSE econ? lol

	Now, we know that a linear equivalence between two finite-dimensional vector spaces.
	Then we have $\dim \mathbb{k}^n = \dim \mathbb{k}^m$ or $n = m$.
...
\begin{center}
	\begin{tikzcd}
		V \arrow[swap, d, "S", hook, two heads] \arrow[r, "T", hook, two heads] & \mathbb{k}^m \\
		\mathbb{k}^n \arrow[ur, "TS^{-1}", hook, two heads, swap]
	\end{tikzcd}
\end{center}
\begin{remark}
...
\begin{definition}[Elementary Column Operations]
	Let $A$ be an $m \times n$ matrix over a field $\mathbb{k}$.
	An \emph{elementary column operation}\index{elementary column operation} on $A$ is one of the following operations:
	\begin{enumerate}
		\item Column Interchange: \qquad $C_i \leftrightarrow C_j$.
		\item Column Multiplication: \quad $C_i \to \alpha C_i$, where $\alpha \in \mathbb{k} \setminus \{0\}$.
		\item Column Addition: \qquad\quad\ $C_i \to C_i + \alpha C_j$, where $\alpha \in \mathbb{k}$ and $i \neq j$.
	\end{enumerate}
	Each elementary column operation can be represented by \emph{right multiplication}\index{right multiplication} of $A$ by an appropriate $n \times n$ matrix over $\mathbb{k}$.
	Note that all of them are invertible linear maps from $\mathbb{k}^{m \times n}$ to $\mathbb{k}^{m \times n}$.
\end{definition}
...
\begin{center}
	\begin{tikzcd}
		\mathbb{k}^n \arrow[r, "A", hook, two heads] \arrow[d, "Q", hook, two heads] & \mathbb{k}^m \arrow[d, "P", hook, two heads] \\
		\mathbb{k}^n \arrow[r, "B", hook, two heads] & \mathbb{k}^m
	\end{tikzcd}
\end{center}
...
\begin{definition}[Canonical Form of a Matrix]
	The matrix $\begin{bmatrix}
		I_r & 0 \\
		0 & 0
	\end{bmatrix}$ obtained from an $m \times n$ matrix $A$ by a finite sequence of elementary row and column operations on $A$ is called the \emph{canonical form}\index{canonical form} of $A$.
\end{definition}
\begin{remark}
	The canonical form of a matrix defined is also called the \emph{Smith Normal Form}\index{Smith Normal Form} or \emph{Normal Form}\index{Normal Form} of a matrix.
\end{remark}

\begin{definition}[Rank of a Matrix]
	The \emph{rank}\index{rank} of an $m \times n$ matrix $A$ over a field $\mathbb{k}$, denoted by $\rank(A)$, is the number of leading 1's in the matrix $\begin{bmatrix}
		I_r & 0 \\
		0 & 0
...
\begin{proof}
	...
	Also note that the rank is invariant under multiplication by invertible matrices, so $\rank(A) = \rank(PAQ^{-1)}$.
	Hence, $\rank(A) = m = n$.
...
\begin{proof}
	...
	\[
		...
		\end{bmatrix}
	\]
	where $\vec{y_1}$ and $\vec{y_2}$ are column vectors with size $r$ and $n - r$ respectively.
	...
	Also, $Q$ is invertible, hence $\vec y$ has only one trivial solution $\vec 0$, i.e., $\vec y_2 = \vec 0$.
	Hence we have $n - r = 0$ due to the size of $\vec y_2$ being 0.
...
\begin{center}
	\begin{tikzcd}
		...
		\mathbb{k}^n \arrow[d, hook, two heads, "Q" swap] \arrow[r, "A", hook, two heads] & \mathbb{k}^m \arrow[d, hook, two heads, "P"] \\
		\mathbb{k}^n \arrow[r, "C", hook, two heads] & \mathbb{k}^m
	\end{tikzcd}
\end{center}
...
Let $f : V \to W$ be a linear map between two finite dimensional vector spaces over $\mathbb{k}$.
